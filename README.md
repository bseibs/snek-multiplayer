...
A snake game that initializes its user (me) as BDS (my initials). It allows you to use the controls A,W,D,S to move around, but cannot move back on itself 180 degrees.
the H key will sending a small "hi" text greeting and simply add more within the code if nexessary.

There is a block of code commented out within the client.js file. This block of code intializes 2 snakes, both the name BDS. If multiplayer, will need to add the arrow functions to the movement patterns for the keys to be used on the second snake.
